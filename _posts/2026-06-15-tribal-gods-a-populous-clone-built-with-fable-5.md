...
<style>
  ...
  /* Steam store widget — same component the ban post uses; kramdown escapes
     raw <iframe>, so the iframe is injected by the small script at the end. */
...
<script>
// Inject the Steam store widget. Kramdown escapes raw <iframe> in the page
// source, so it's built here instead — same approach as the ban post.
(function () {
